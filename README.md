...
```
Codex CLI ──Responses API──▶ 本代理 (:18035) ──Chat API──▶ DeepSeek
```
...
```bash
git clone https://github.com/arronfan23/codex-deepseek-bridge.git
...
3. **关键**：将请求地址改为 `http://127.0.0.1:18035`
4. 勾选「写入通用配置」→ 添加 → 启用
...
```bash
# Windows
netstat -ano | findstr 18035
taskkill /F /PID <PID>

# macOS/Linux
lsof -ti:18035 | xargs kill -9
```
...
本代理做的事情：
1. 在 `127.0.0.1:18035` 起一个 HTTP 服务
